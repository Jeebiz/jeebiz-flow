<template>
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
      >
      <el-radio-group v-model="czfs">
        <el-radio v-for="(item, index) in items" :key="index" :label="item.label">
          {{item.title}}
        </el-radio>
      </el-radio-group>

      <span slot="footer">
        <el-button style="width: 90px;" @click="dialog = false">取 消</el-button>
        <el-button style="width: 90px;" type="primary" @click="confirmChange">确 定</el-button>
      </span>
    </el-dialog>
...
export default {
	...
	data () {
      ...
      qxzList: [],
        ...
        czfs: 0,
        items: [
          {label: 0, title: '系统默认密码'},
          {label: 1, title: '身份证后6位'},
          {label: 2, title: '手机号后6位'},
          {label: 3, title: '学工号后6位'}
        ],
				title: '',
				pager: {
					total: 0,
      ...
      API.specialUserPairs(data).then(res => {
        if(res.code == 0) {
    ...
    // 确定初始化密码
    confirmChange () {
      this.$confirm('如果所选角色的身份证号码,手机号或者学工号没有录入系统,则会初始化为系统默认密码', '提示', {
